...
import { Send, Mic, Copy, MoreHorizontal, Bot, User, Loader2 } from "lucide-react";
...
export default function ChatPage() {
  const { messages, input, handleInputChange, handleSubmit, isLoading } = useChat({
    api: "/api/chat",
    initialMessages: [
      { id: "1", role: "assistant", content: "Hello! I'm your Planner Agent. How can I assist you with your business strategy today?" }
    ]
  });
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)] max-w-4xl mx-auto bg-white/40 backdrop-blur-3xl rounded-[32px] shadow-[0_20px_60px_rgba(79,124,255,0.08)] border border-white/60 overflow-hidden relative">
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-8 z-10 custom-scrollbar">
            ...
            <div className={`group relative max-w-[80%] p-5 rounded-[24px] ${msg.role === 'user' ? 'bg-[#0A121A]/5 text-[#0A121A] rounded-tr-sm border border-white/40 backdrop-blur-sm' : 'bg-white/60 border border-white/60 text-[#0A121A] rounded-tl-sm backdrop-blur-md shadow-sm'}`}>
              ...
              {msg.role === 'assistant' && (
                <button 
                  ...
                  className="absolute -bottom-4 -right-4 opacity-0 group-hover:opacity-100 p-2.5 bg-white rounded-full shadow-[0_8px_20px_rgba(79,124,255,0.15)] border border-white/60 text-[#0A121A]/40 hover:text-[#4F7CFF] transition-all transform hover:scale-110"
                >
        ...
        {isLoading && (
           <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-4">
             <div className="w-10 h-10 rounded-[14px] bg-white/80 border-[#4F7CFF]/20 flex items-center justify-center shrink-0 mt-1 shadow-sm border backdrop-blur-md">
      ...
      <div className="p-5 bg-white/40 backdrop-blur-xl border-t border-white/60 z-10">
        <form onSubmit={handleSubmit} className="relative max-w-4xl mx-auto flex items-center bg-white/60 backdrop-blur-md rounded-[20px] border border-white/60 focus-within:border-[#4F7CFF] focus-within:ring-4 focus-within:ring-[#4F7CFF]/10 transition-all p-2 shadow-sm">
          <button type="button" className="p-3 text-[#0A121A]/40 hover:text-[#7B5CFF] transition-colors rounded-xl hover:bg-white">
